...
class MecanumChassis {
   public:
    ...
    ros_robot_controller_msgs::msg::MotorsState setVelocity(double linear_x, double linear_y, double angular_z) const {
        ...
        // 转换为rps
        std::vector<double> speeds = {speedToRps(motor1), speedToRps(motor2),
...
class SpeedSubscriber : public rclcpp::Node {
   public:
    SpeedSubscriber() : Node("speed_subscriber"), chassis_(0.216, 0.195, 0.097) {
        ...
        cmd_vel_subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
            "cmd_vel", 10, [this](const geometry_msgs::msg::Twist::SharedPtr msg) { this->cmdVelCallback(msg); });
   ...
   private:
    ...
    void cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
        ...
        // 打印日志
        RCLCPP_INFO(this->get_logger(), "收到速度指令: 线速度(%.2f, %.2f) m/s, 角速度%.2f rad/s", linear_x, linear_y,
                    angular_z);

        RCLCPP_INFO(this->get_logger(), "发布电机转速: 1:%.2f, 2:%.2f, 3:%.2f, 4:%.2f rps", motors_msg.data[0].rps,
                    motors_msg.data[1].rps, motors_msg.data[2].rps, motors_msg.data[3].rps);
    }
...
class MecanumChassis {
   public:
    ...
    void configure(double wheelbase, double track_width, double wheel_diameter) {
...
class SpeedSubscriber : public rclcpp::Node {
   public:
    SpeedSubscriber() : Node("speed_subscriber"), chassis_(0.216, 0.195, 0.097) {
        ...
        this->declare_parameter<std::string>("motor_topic", "ros_robot_controller/set_motor");
        this->declare_parameter<double>("wheelbase", 0.216);
        this->declare_parameter<double>("track_width", 0.195);
        this->declare_parameter<double>("wheel_diameter", 0.097);
        this->declare_parameter<double>("max_linear_x", 0.5);
        this->declare_parameter<double>("max_linear_y", 0.2);
        ...
        this->get_parameter("motor_topic", motor_topic_);
        ...
        chassis_.configure(wheelbase_, track_width_, wheel_diameter_);

        cmd_vel_subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
            command_topic_, static_cast<size_t>(queue_depth_),
            [this](const geometry_msgs::msg::Twist::SharedPtr msg) { this->cmdVelCallback(msg); });

        motor_publisher_ = this->create_publisher<ros_robot_controller_msgs::msg::MotorsState>(
            motor_topic_, static_cast<size_t>(queue_depth_));

        RCLCPP_INFO(this->get_logger(), "速度订阅节点已启动，订阅 %s，发布 %s", command_topic_.c_str(),
                    motor_topic_.c_str());
    }
   ...
   private:
    ...
    void cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
        ...
        if (linear_x > max_linear_x_) linear_x = max_linear_x_;
        if (linear_x < -max_linear_x_) linear_x = -max_linear_x_;
        if (linear_y > max_linear_y_) linear_y = max_linear_y_;
        if (linear_y < -max_linear_y_) linear_y = -max_linear_y_;
        if (angular_z > max_angular_z_) angular_z = max_angular_z_;
        if (angular_z < -max_angular_z_) angular_z = -max_angular_z_;
