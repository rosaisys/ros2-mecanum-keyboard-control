...
public:
    ...
    explicit SpeedPublisher(const std::string& robot_name = "smart_car")
        ...
        RCLCPP_INFO(this->get_logger(), 
                   ...
                   machine_type_.c_str(), robot_name.c_str());
    ...
    void runControlLoop() {
        ...
        while (rclcpp::ok() && !should_stop_ && !KeyboardInput::shouldExit()) {
            ...
            // 速度平滑处理：逐步接近目标速度
            smoothVelocity(target_linear_vel_, target_angular_vel_, 
...
private:
     ...
     */
    void smoothVelocity(double target_linear_vel, double target_angular_vel,
    ...
    void publishVelocityIfChanged() {
        ...
        if (linear_changed || angular_changed || control_angular_vel_ != 0.0) {
            ...
            // 显示当前速度
            std::cout << "\r[控制指令] 线速度: " << std::fixed << std::setprecision(2) 
                      << twist->linear.x << " m/s | 角速度: " 
                      << twist->angular.z << " rad/s        " << std::flush;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 在单独线程中运行控制循环
        std::thread control_thread([speed_node]() {
            speed_node->runControlLoop();
        });
